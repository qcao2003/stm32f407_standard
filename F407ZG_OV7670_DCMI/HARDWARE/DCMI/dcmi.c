...
{  
	DCMI_Stop();
	LCD_Display_Dir(sytle); //0竖屏方向,1是横屏方向
	LCD_Set_Window(sx,sy,width,height);
	//OV7670_Window_Set(0,0,width,height);
	//OV7670_config_window(sx,sy,width,height);
	//OV7670_Config_320_240();
  LCD_SetCursor(sx,sy);  
...
void DCMI_Set_Window(u16 sx,u16 sy,u16 width,u16 height)
{
	...
	LCD_Clear(WHITE);
	LCD_Set_Window(sx,sy,width,height);
	OV7670_Window_Set(4,0,width,height);
  LCD_SetCursor(sx,sy);  
	LCD_WriteRAM_Prepare();		//开始写入GRAM
...
{ 
  ...
  POINT_COLOR=RED; 
	...
	DCMI_Start(0,0,320,240,1);  //启动传输并输入显示屏幕参数
